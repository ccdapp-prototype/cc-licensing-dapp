...
library Bytes {
    ...
    for (uint256 i = 0; i < buffer.length; i += 0x20) {
        ...
        if (chunk != bytes32(0)) {
            // Convert bytes32 to bytes before passing to clz
            bytes memory chunkBytes = abi.encodePacked(chunk);
            return Math.min(8 * i + uint256(clz(chunkBytes)), 8 * buffer.length);
        }
    }
...
// File: contracts/CCassetLicensing.sol
